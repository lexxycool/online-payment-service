...
public class JdbcUserDao implements UserDao {
    ...
    public BigDecimal updateReceiverBalance(int accountID, BigDecimal amount){
        String sql = "select balance from accounts where account_id = ?";
        SqlRowSet rowSet = jdbcTemplate.queryForRowSet(sql,accountID);
        BigDecimal bigDecimal = new BigDecimal("0.00");
        if(rowSet.next()){
            bigDecimal = rowSet.getBigDecimal("balance");
            bigDecimal = (bigDecimal.add(amount));
            jdbcTemplate.update("update accounts set balance = ? where account_id = ?", bigDecimal, accountID);
        }

    return bigDecimal;
    }
    public BigDecimal updateSenderBalance(int accountID, BigDecimal amount){
        String sql = "select balance from accounts where account_id = ?";
        SqlRowSet rowSet = jdbcTemplate.queryForRowSet(sql,accountID);
        BigDecimal bigDecimal = new BigDecimal("0.00");
        if(rowSet.next()){
